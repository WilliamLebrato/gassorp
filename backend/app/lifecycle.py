...
class LifecycleManager:
    def __init__(self, docker_manager: SidecarManager, db_session_factory):
    ...
    async def _check_idle_servers(self):
        with self.get_session() as session:
            ...
            for server in running_servers:
                ...
                try:
                    stats = self.docker_manager.get_container_stats(server)
                    ...
                    if stats['cpu_percent'] < 5.0:
                        ...
                        if idle_time >= timedelta(minutes=15):
                            ...
                            if self.docker_manager.hibernate(server):
        ...
        with self.get_session() as session:
            ...
            for server in running_servers:
                ...
                if user.credits <= 0:
                    ...
                    self.docker_manager.hibernate(server)
    ...
    async def wake_on_webhook(self, server_id: int, token: str) -> bool:
        if token != self.docker_manager.webhook_secret:
        ...
        with self.get_session() as session:
            ...
            if self.docker_manager.wake(server):
